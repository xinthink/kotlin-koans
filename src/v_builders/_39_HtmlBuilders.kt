...
fun renderProductTable(): String {
    return html {
        table {
            ...
            products.forEachIndexed { i, (description, price, popularity) ->
                tr {
                    td(color = getCellColor(row = i, column = 0)) {
                        text(description)
                    }
                    td(color = getCellColor(row = i, column = 1)) {
                        text(price)
                    }
                    td(color = getCellColor(row = i, column = 2)) {
                        text(popularity)
                    }
                }
            }
